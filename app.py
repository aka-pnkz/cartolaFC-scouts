...
st.markdown("""
    ...
    """, unsafe_allow_state_warnings=True)
...
def get_all_data():
    try:
        ...
        st.error("⚠️ Erro ao conectar com a API do Cartola. Tente novamente mais tarde.")
        return None, None
...
def optimize_team(df_pool, budget, formation):
    # Configuração de slots por formação
    configs = {
    ...
    # 2. Ajuste de Orçamento (Downgrade de custo)
    selected_df = pd.DataFrame(selected)
    while current_cost > budget:
        # Tenta trocar o mais caro que não seja o técnico
        ...
        for i in range(len(selected_df)):
            ...
            # Busca substituto mais barato na mesma posição
            ...
            if not sub.empty:
                ...
                break
        else: break # Se não houver mais trocas possíveis
            
    return selected_df, round(current_cost, 2)

# --- APP PRINCIPAL ---
def main():
    st.sidebar.image("https://s3.glbimg.com/v1/AUTH_58d78b787ec34892b5aaa0c7a146155f/clubes_2026/escudos/FLA/60x60.png", width=100) # Logo genérico
    st.sidebar.title("MENU MITADA")
    ...
    st.caption("Arquitetura de Dados para Alta Performance no Cartola FC")
    ...
    if mercado_raw:
        ...
        with col_t1:
            st.dataframe(time_ideal[['pos_nome', 'apelido', 'clube_nome', 'mando', 'media_num', 'preco_num', 'min_para_valorizar', 'score_mitada']], 
                         use_container_width=True, hide_index=True)
        
        with col_t2:
            st.metric("Custo Total", f"C$ {custo_final}", f"{saldo_disp - custo_final:.2f} sobra")
            capitao = time_ideal.sort_values('score_mitada', ascending=False).iloc[0]
            st.success(f"💎 **Capitão:** {capitao['apelido']}")
            st.info(f"O capitão foi escolhido pelo Score de {capitao['score_mitada']} pts.")
        ...
        for pos in ['GOL', 'LAT', 'ZAG', 'MEI', 'ATA']:
            if pos in config_da_pos := time_ideal['pos_nome'].unique():
                ...
                if not res.empty: reservas.append(res.iloc[0])
        ...
        with c1:
            ...
            st.write("Quem precisa de menos pontos para subir de preço.")
        ...
        with c2:
            st.subheader("🎯 Potencial Ofensivo")
            fig = px.scatter(df_prov[df_prov['pos_nome'] == 'ATA'].head(15), 
                             x="media_num", y="score_mitada", text="apelido",
                             title="Média x Score de Mitada (Atacantes)")
            st.plotly_chart(fig, use_container_width=True)

        # --- SEÇÃO 4: CONFRONTOS ---
        with st.expander("🏟️ Calendário da Rodada"):
            for p in partidas:
                c = clubes_map[str(p['clube_casa_id'])]['nome']
                f = clubes_map[str(p['clube_visitante_id'])]['nome']
                st.write(f"**{c}** 🏠 x ✈️ **{f}**")
